import { default as ArrowIcon } from "./Arrow.icon";
import { default as ArrowDotIcon } from "./ArrowDot.icon";
import { default as CheckDoneIcon } from "./CheckDone.icon";
import { default as ClockIcon } from "./Clock.icon";
import { default as DropdownArrowIcon } from "./DropdownArrow.icon";
import { default as ExchangeIcon } from "./Exchange.icon";
import { default as FeedIcon } from "./Feed.icon";
import { default as MacKeyIcon } from "./MacKey.icon";
import { default as MessageIcon } from "./Message.icon";
import { default as NotifyIcon } from "./Notify.icon";
import { default as PlusIcon } from "./Plus.icon";
import { default as ResearchIcon } from "./Research.icon";
import { default as RoomsIcon } from "./Rooms.icon";
import { default as SearchIcon } from "./Search.icon";
import { default as SendIcon } from "./Send.icon";
import { default as TokenIcon } from "./Token.icon";
import { default as WrenchIcon } from "./Wrench.icon";

export {
   DropdownArrowIcon,
   PlusIcon,
   WrenchIcon,
   RoomsIcon,
   ResearchIcon,
   FeedIcon,
   MacKeyIcon,
   SearchIcon,
   NotifyIcon,
   CheckDoneIcon,
   ExchangeIcon,
   ClockIcon,
   SendIcon,
   ArrowDotIcon,
   MessageIcon,
   TokenIcon,
   ArrowIcon,
};
